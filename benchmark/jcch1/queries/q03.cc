...
std::unique_ptr<Operator> SelectCustomer() {
  ...
  auto eq =
      Exp(Eq(VirtColRef(scan_schema, "c_mktsegment"), Literal("BUILDING"sv)));
...
std::unique_ptr<Operator> SelectOrders() {
  ...
  auto lt = Lt(ColRef(scan_orders, "o_orderdate"), Literal(1993, 5, 29));
...
std::unique_ptr<Operator> SelectLineitem() {
  ...
  auto gt = Gt(ColRef(lineitem, "l_shipdate"), Literal(1993, 5, 29));
