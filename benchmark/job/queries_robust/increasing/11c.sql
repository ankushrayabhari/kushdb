...
FROM company_name AS cn,
     ...
     title AS t WHERE ct.kind IS NOT NULL AND ct.kind <> 'production companies' AND ct.kind IS NOT NULL AND ct.kind IS NOT NULL AND ct.kind IS NOT NULL AND ct.kind IS NOT NULL AND ct.kind IS NOT NULL AND k.keyword IN ('sequel',
                    ...
                    'based-on-novel') AND lt.id >= 0 AND lt.id >= 0 AND lt.id >= 0 AND lt.id >= 0 AND lt.id >= 0 AND (cn.name LIKE '20th Century Fox%'

       OR cn.name LIKE 'Twentieth Century Fox%') AND cn.country_code <> '[pl]' AND (cn.name LIKE '20th Century Fox%'

       OR cn.name LIKE 'Twentieth Century Fox%') AND (cn.name LIKE '20th Century Fox%'
