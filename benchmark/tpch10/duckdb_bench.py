...
def bench(db, use_indexes):
    cmd_base = ['.open benchmark/tpch10/duckdb/tpch.ddb']
    if use_indexes:
        cmd_base.append('.read benchmark/tpch10/indexes.sql')
    ...
    for query in queries:
        ...
        cmds = cmd_base.copy()
        for _ in range(6):
            cmds.append('.read benchmark/tpch10/queries/' + query + '.sql')
        execute_duckdb(cmds)
        with open('/tmp/bench_time.txt', 'r') as file:
            ...
            for t in data.split():
                try:
                    ...
                    continue
            for t in times[3::3]:
                print(db, 'TPC-H SF10', query_num, t * 1000, sep=',')
