...
// Select(p_size = 27 and p_type ENDS WITH 'BRASS')
std::unique_ptr<Operator> SelectPart() {
  ...
  auto ends_with =
      Exp(EndsWith(VirtColRef(scan_schema, "p_type"), Literal("BRASS"sv)));
  auto eq = Exp(Eq(VirtColRef(scan_schema, "p_size"), Literal(27)));
