...
def bench():
    ...
    for query in queries:
        cmd = 'mclient --timer=performance -d jcch benchmark/jcch10/queries/' + query + '.sql > /dev/null 2> /dev/null'
        execute(cmd)
    # execute 5 times
    for query in queries:
        query_num = int(query[1:])
        cmd = 'mclient --timer=performance -d jcch benchmark/jcch10/queries/' + query + '.sql > /dev/null 2> /tmp/bench_time.txt'
        for _ in range(5):
            execute(cmd)
            with open('/tmp/bench_time.txt', 'r') as file:
                ...
                for t in data.split():
                    try:
                        ...
                        continue
                print('MonetDB', 'JCC-H SF10', query_num, times[-1], sep=',')
