...
// Select(l_shipdate >= '1993-08-01' and l_receiptdate < '1993-09-01')
std::unique_ptr<Operator> SelectLineitem() {
  auto lineitem = ScanLineitem();

  std::unique_ptr<Expression> p1 =
      Geq(ColRef(lineitem, "l_shipdate"), Literal(absl::CivilDay(1993, 8, 1)));
  std::unique_ptr<Expression> p2 =
      Lt(ColRef(lineitem, "l_shipdate"), Literal(absl::CivilDay(1993, 9, 1)));
...
int main(int argc, char** argv) {
  ...
  BenchVerify(*query, "benchmark/jcch10/raw/q14.tbl");
  return 0;
}
