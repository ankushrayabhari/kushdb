...
// Select(n_name in 'GERMANY' or n_name in 'MOROCCO')
std::unique_ptr<Operator> SelectNation() {
  ...
  auto cond = Exp(Or(util::MakeVector(
      Exp(Eq(VirtColRef(scan_schema, "n_name"), Literal("GERMANY"sv))),
      Exp(Eq(VirtColRef(scan_schema, "n_name"), Literal("MOROCCO"sv))))));
...
std::unique_ptr<Operator> SelectLineitem() {
  ...
  auto le =
      Exp(Leq(VirtColRef(scan_schema, "l_shipdate"), Literal(1994, 12, 31)));
...
//   (n1.n_name = 'GERMANY' and n2.n_name = 'MOROCCO')
//   or (n1.n_name = 'MOROCCO' and n2.n_name = 'GERMANY')
// )
std::unique_ptr<Operator> Join() {
      ...
      Eq(ColRef(customer, "c_nationkey", 3), ColRef(n2, "n_nationkey", 5)));
  {
    std::unique_ptr<Expression> eq1 =
        Eq(ColRef(n1, "n_name", 4), Literal("GERMANY"sv));
    std::unique_ptr<Expression> eq2 =
        Eq(ColRef(n2, "n_name", 5), Literal("MOROCCO"sv));
    std::unique_ptr<Expression> eq3 =
        Eq(ColRef(n1, "n_name", 4), Literal("MOROCCO"sv));
    std::unique_ptr<Expression> eq4 =
        Eq(ColRef(n2, "n_name", 5), Literal("GERMANY"sv));
