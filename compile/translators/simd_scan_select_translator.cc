...
void SimdScanSelectTranslator::Produce() {
  ...
  proxy::Loop loop(
      ...
      [&](auto& loop) {
        ...
        auto buffer_type = program_.ArrayType(program_.I32Type(), BUFFER_SIZE);
        auto buffer = program_.StaticGEP(
            buffer_type,
            program_.Global(
                buffer_type,
                program_.ConstantArray(buffer_type,
            ...
            [&]() {
              proxy::Loop manual_loop(
                  ...
                  [&](auto& manual_loop) {
                    ...
                    return manual_loop.Continue(tuple_idx + 1, buffer_size + 1);
                  });
