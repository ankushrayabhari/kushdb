...
proxy::Int32 RecompilingSkinnerJoinTranslator::GenerateChildLoops(
        ...
        [&]() -> std::vector<khir::Value> {
          ...
          auto progress_check = proxy::If(
              ...
              [&]() -> std::vector<khir::Value> {
                ...
                auto bucket_idx = bucket.FastForwardToStart(last_tuple);
                auto should_continue_resume_progress = proxy::If(
                    program, bucket_idx < bucket_size,
                    [&]() -> std::vector<khir::Value> {
                      return {(last_tuple == bucket.Get(bucket_idx)).Get()};
                    },
                    [&]() -> std::vector<khir::Value> {
                      return {proxy::Bool(program, false).Get()};
                    });
                return {bucket_idx.Get(), should_continue_resume_progress[0]};
              },
              [&]() -> std::vector<khir::Value> {
                proxy::Int32 bucket_idx(program, 0);
                proxy::Bool continue_resume_progress(program, false);
                return {bucket_idx.Get(), continue_resume_progress.Get()};
              });
          proxy::Int32 initial_bucket_idx(program, progress_check[0]);
          proxy::Bool continue_resume_progress(program, progress_check[1]);

          auto exit_early_check = proxy::If(
              program, initial_bucket_idx >= bucket_size,
              [&]() -> std::vector<khir::Value> {
                auto budget = initial_budget - 1;
                proxy::If(
                    program, budget == 0,
              ...
              [&]() -> std::vector<khir::Value> {
                proxy::Loop loop(
                    program,
                    [&](auto& loop) {
                      loop.AddLoopVariable(initial_bucket_idx);
                      ...
                      if (curr + 1 == order.size()) {
                        ...
                        next_budget =
                            GenerateChildLoops(
                                ...
                                num_result_tuples_ptr, budget,
                                continue_resume_progress)
                                .ToPointer();
                      }
          ...
          proxy::Int32 next_budget(program, exit_early_check[0]);
          return {next_budget.Get()};
        });
  ...
  proxy::Loop loop(
      ...
      [&](auto& loop) {
        auto last_tuple = loop.template GetLoopVariable<proxy::Int32>(0);
        auto last_budget = loop.template GetLoopVariable<proxy::Int32>(1);
        ...
        auto progress_check = proxy::If(
            ...
            [&]() -> std::vector<khir::Value> {
              ...
              auto next_tuple =
                  proxy::Int32(program, program.LoadI32(progress_ptr));
              return {next_tuple.Get(), last_budget.Get()};
            },
            [&]() -> std::vector<khir::Value> {
              auto next_tuple = last_tuple + 1;
              auto next_budget = last_budget - 1;
              return {next_tuple.Get(), next_budget.Get()};
            });

        // write idx into idx array
        auto next_tuple = proxy::Int32(program, progress_check[0]);
        auto budget = proxy::Int32(program, progress_check[1]);
        auto idx_ptr =
