...
template <typename T>
IndexLoop<T>::IndexLoop(
    ProgramBuilder<T>& program, std::function<UInt32<T>()> init,
    std::function<Bool<T>(UInt32<T>&)> cond,
  ...
  auto continue_code = [&](UInt32<T>& next) {
    program.AddToPhi(phi, next.Get(), program.CurrentBlock());
    program.Branch(header_block);
  };
  ...
  // jump back to header
  program.SetCurrentBlock(body_block);
  auto updated_value = body(loop_var, continue_code);
  program.AddToPhi(phi, updated_value.Get(), program.CurrentBlock());
  program.Branch(header_block);
