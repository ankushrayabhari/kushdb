...
std::unique_ptr<Operator> SelectNation() {
  ...
  std::vector<std::vector<std::unique_ptr<BinaryArithmeticExpression>>> filters(
      2);
  filters[1].push_back(
      Eq(VirtColRef(scan_schema, "n_name"), Literal("EGYPT"sv)));
  filters[1].push_back(
      Eq(VirtColRef(scan_schema, "n_name"), Literal("KENYA"sv)));
...
std::unique_ptr<Operator> SelectLineitem() {
  ...
  std::vector<std::vector<std::unique_ptr<BinaryArithmeticExpression>>> filters(
      5);
  filters[0].push_back(
      Geq(VirtColRef(scan_schema, "l_shipdate"), Literal(1995, 1, 1)));
  filters[0].push_back(
      Leq(VirtColRef(scan_schema, "l_shipdate"), Literal(1996, 12, 31)));
