...
TEST_P(OrderByTest, SmallIntCol) {
  ...
  std::unique_ptr<Operator> query;
  {
    std::unique_ptr<Operator> base;
    {
      ...
      schema.AddGeneratedColumns(db["info"], {"num1", "id"});
      base = std::make_unique<ScanOperator>(std::move(schema), db["info"]);
    }

    auto num1 = ColRef(base, "num1");
    ...
    query = std::make_unique<OutputOperator>(std::make_unique<OrderByOperator>(
        ...
        util::MakeVector(std::move(num1), std::move(id)),
        std::vector<bool>{asc, asc}));
  }

  auto expected_file = "end_to_end_test/order_by/smallint_expected.tbl";
