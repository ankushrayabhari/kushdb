...
TEST_P(SelectTest, TextCol) {
  ...
  std::unique_ptr<Operator> query;
  {
    ...
    std::vector<std::vector<std::unique_ptr<BinaryArithmeticExpression>>>
        filters(2);
    filters[0].push_back(
        Leq(VirtColRef(scan_schema, "l_shipdate"), Literal(1994, 12, 31)));
    filters[0].push_back(
        Geq(VirtColRef(scan_schema, "l_shipdate"), Literal(1994, 1, 1)));
    filters[1].push_back(
  ...
  auto expected_file =
      "end_to_end_test/simd_scan_select/multiple_col_expected.tbl";
