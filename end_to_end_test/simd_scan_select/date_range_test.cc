...
TEST_P(SelectTest, TextCol) {
  ...
  std::unique_ptr<Operator> query;
  {
    ...
    scan_schema.AddGeneratedColumns(db["lineitem"], {"l_shipdate"});

    std::vector<std::vector<std::unique_ptr<BinaryArithmeticExpression>>>
        filters(1);
    filters[0].push_back(
        Leq(VirtColRef(scan_schema, "l_shipdate"), Literal(1998, 12, 31)));
    filters[0].push_back(
        Geq(VirtColRef(scan_schema, "l_shipdate"), Literal(1998, 1, 1)));

    OperatorSchema schema;
    schema.AddVirtualPassthroughColumns(scan_schema, {"l_shipdate"});
  ...
  auto expected_file =
      "end_to_end_test/simd_scan_select/date_range_expected.tbl";
