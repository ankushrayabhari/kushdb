...
TEST_P(SelectTest, TextCol) {
  ...
  std::unique_ptr<Operator> query;
  {
    ...
    scan_schema.AddGeneratedColumns(db["lineitem"], {"l_returnflag"});

    std::vector<std::vector<std::unique_ptr<BinaryArithmeticExpression>>>
        filters(1);
    filters[0].push_back(
        Eq(VirtColRef(scan_schema, "l_returnflag"), Literal("R"sv)));

    OperatorSchema schema;
    schema.AddVirtualPassthroughColumns(scan_schema, {"l_returnflag"});
  ...
  auto expected_file = "end_to_end_test/simd_scan_select/enum_expected.tbl";
