...
TEST_P(BackendTest, I64_XORConstArg0) {
  std::random_device rd;
  std::mt19937 gen(rd());
  std::uniform_int_distribution<int64_t> distrib(INT64_MIN, INT64_MAX);
...
TEST_P(BackendTest, I64_XORConstArg1) {
  std::random_device rd;
  std::mt19937 gen(rd());
  std::uniform_int_distribution<int64_t> distrib(INT64_MIN, INT64_MAX);
...
TEST_P(BackendTest, I32Vec8OR) {
  ...
  auto args = program.GetFunctionArguments(func);
  auto v1 = program.LoadI32Vec8(args[0]);
  auto mask1 = program.CmpI32Vec8(CompType::EQ, v1, program.ConstI32Vec8(3));
  auto mask2 = program.CmpI32Vec8(CompType::EQ, v1, program.ConstI32Vec8(4));
...
TEST_P(BackendTest, I32Vec8And) {
  ...
  auto args = program.GetFunctionArguments(func);
  auto v1 = program.LoadI32Vec8(args[0]);
  auto mask1 = program.CmpI32Vec8(CompType::LE, v1, program.ConstI32Vec8(4));
  auto mask2 = program.CmpI32Vec8(CompType::GE, v1, program.ConstI32Vec8(4));
