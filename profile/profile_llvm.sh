#!/bin/bash
# Example: ./profile/profile_llvm.sh bazel-bin/benchmark/tpch/q02_skinner --skinner=permute
QUERY=$1
shift
bazel build -c opt --config=profile //...
sudo perf record -F 5000 -k 1 -g --call-graph fp -o /tmp/perf.data $QUERY --backend=llvm $@
