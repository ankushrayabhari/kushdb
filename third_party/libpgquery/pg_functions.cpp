...
void *palloc(size_t n) {
  ...
  void *ptr = pg_parser_state.malloc_ptrs[pg_parser_state.malloc_ptr_idx - 1] +
...
void pg_parser_parse(const char *query, parse_result *res) {
  ...
  try {
    res->parse_tree = libpgquery::raw_parser(query);
...
void pg_parser_cleanup() {
  for (size_t ptr_idx = 0; ptr_idx < pg_parser_state.malloc_ptr_idx;
       ptr_idx++) {
...
void *repalloc(void *ptr, size_t n) { return palloc(n); }
