 ...
 *
 *	libpgquery::T_PGList: lists of pointers
 *		(in practice usually pointers to Nodes, but not always;
 *		declared as "void *" to minimize casting annoyances)
 *	libpgquery::T_PGIntList: lists of integers
 *	libpgquery::T_PGOidList: lists of Oids
 *
...
typedef struct PGList {
  PGNodeTag type; /* libpgquery::T_PGList, libpgquery::T_PGIntList, or
                     libpgquery::T_PGOidList */
  int length;
  PGListCell *head;
  PGListCell *tail;
} PGList;
 ...
 * arguments.
 */
static inline PGListCell *list_head(const PGList *l) {
 ...
 *	  specified cells of each. This macro loops through both lists at the
 *same time, stopping when either list runs out of elements.  Depending on the
