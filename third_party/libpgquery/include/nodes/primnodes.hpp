 ...
 *
 * Note: colnames is a list of PGValue nodes (always strings).  In PGAlias
 *structs associated with RTEs, there may be entries corresponding to dropped
 ...
 * an index column rather than a heap column.  (In PGForeignScan and
 * PGCustomScan plan nodes, INDEX_VAR is abused to signify references to columns
 * of a custom scan tuple type.)  In all these cases, varnoold/varoattno hold
 * the original values.  The code doesn't really need varnoold/varoattno, but
 ...
 *		for this field are:
 *
 *		PG_PARAM_EXTERN:  The parameter value is supplied from outside the
 *plan. Such parameters are numbered from 1 to n.
 *
 *		PG_PARAM_EXEC:  The parameter is an internal executor parameter,
 *used for passing values into and out of sub-queries or from nestloop joins to
 *their inner scans. For historical reasons, such parameters are numbered from
 *0. These numbers are independent of PG_PARAM_EXTERN numbers.
 *
 *		PG_PARAM_SUBLINK:	The parameter represents an output column
 *of a PGSubLink node's sub-select.  The column number is contained in the
 *				`paramid' field.  (This type of PGParam is converted
 *to PG_PARAM_EXEC during planning.)
 *
 *		PG_PARAM_MULTIEXPR:  Like PG_PARAM_SUBLINK, the parameter
 *represents an output column of a PGSubLink node's sub-select, but here, the
 *				PGSubLink is always a MULTIEXPR SubLink.  The
 *high-order 16 bits of the `paramid' field contain the SubLink's subLinkId, and
 *				the low-order 16 bits contain the column number.
 *(This type of PGParam is also converted to PG_PARAM_EXEC during planning.)
 */
 ...
 * that one PGAgg node can handle multiple grouping sets at once.  Evaluating
 * the result only needs the column positions to check against the grouping set
 ...
 *...) PG_CTE_SUBLINK			WITH query (never actually part of an
 ...
 *target then the parser will create an PG_EXPR_SUBLINK instead (and any
 *operator above the subselect will be represented separately). ROWCOMPARE,
 *EXPR, and MULTIEXPR require the subselect to deliver at most one row (if it
 *returns no rows, the result is NULL). ALL, ANY, and ROWCOMPARE require the
 *combining operators to deliver boolean results.  ALL and ANY combine the
 *per-row results using AND and OR semantics respectively. ARRAY requires just
 *one target column, and creates an array of the target column's type using any
 *number of rows resulting from the subselect.
 *
 ...
 * that compares the lefthand value(s) to PG_PARAM_SUBLINK nodes representing
 *the output columns of the subselect.  And subselect is transformed to a Query.
 ...
 *
 * The PG_CTE_SUBLINK case never occurs in actual PGSubLink nodes, but it is
 *used in SubPlans generated for WITH subqueries.
 */
 ...
 * The values are assigned to the global PG_PARAM_EXEC params indexed by
 * parParam (the parParam and args lists must have the same ordering).  setParam
 * is a list of the PG_PARAM_EXEC params that are computed by the sub-select, if
 * it is an initplan; they are listed in order by sub-select output column
 ...
 *... ] These are distinguishable by the "arg" field being NULL in the first
 *case and the testexpr in the second case.
 *
