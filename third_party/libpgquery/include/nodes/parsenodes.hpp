 ...
 *
 *	  Planning converts a PGQuery tree into a PGPlan tree headed by a
 *PGPlannedStmt node --- the PGQuery structure is not used by the executor.
 */
 ...
 *
 *	  Note that we consider PG_RTE_RELATION to cover anything that has a
 *pg_class entry.  relkind distinguishes the sub-cases.
 *
 *	  alias is an PGAlias node representing the AS alias-clause attached to
 ...
 *	  This flag is not used anymore during parsing, since the parser now
 *uses a separate "namespace" data structure to control visibility, but it is
 ...
 *		query's targetlist; that is the expression to be sorted or grouped
 *by. eqop is the OID of the equality operator. sortop is the OID of the
...
typedef struct PGIntervalConstant {
  PGNodeTag type;
  int val_type;    /* interval constant type, either libpgquery::T_PGString,
                      libpgquery::T_PGInteger or libpgquery::T_PGAExpr */
  char *sval;      /* libpgquery::T_PGString */
  int ival;        /* libpgquery::T_PGString */
  PGNode *eval;    /* libpgquery::T_PGAExpr */
  PGList *typmods; /* how to interpret the interval constant (year, month, day,
                      etc)  */
  int location;    /* token location, or -1 if unknown */
} PGIntervalConstant;
...
typedef struct PGSampleSize {
  PGNodeTag type;
  bool is_percentage;  /* whether or not the sample size is expressed in row
                          numbers or a percentage */
