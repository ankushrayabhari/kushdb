...
static void check_list_invariants(const PGList *list) {
  ...
  if (list->length == 1) Assert(list->head == list->tail);
  if (list->length == 2) Assert(list->head->next == list->tail);
...
PGList *list_concat(PGList *list1, PGList *list2) {
  ...
  if (list1 == list2) elog(ERROR, "cannot list_concat() a list to itself");
...
PGList *list_truncate(PGList *list, int new_size) {
  ...
  if (new_size <= 0) return NIL; /* truncate to zero length */

  /* If asked to effectively extend the list, do nothing */
  if (new_size >= list_length(list)) return list;
...
PGListCell *list_nth_cell(const PGList *list, int n) {
  ...
  if (n == list->length - 1) return list->tail;
...
PGList *list_copy_tail(const PGList *oldlist, int nskip) {
  ...
  if (nskip < 0) nskip = 0; /* would it be better to elog? */

  if (oldlist == NIL || nskip >= oldlist->length) return NIL;
